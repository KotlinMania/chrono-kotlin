# Claude Code Project Instructions — chrono-kotlin

## Project Overview

This is **chrono-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`chrono`](https://crates.io/crates/chrono).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/chrono/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/chrono>
